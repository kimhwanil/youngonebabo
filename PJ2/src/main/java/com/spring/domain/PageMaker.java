...
public class PageMaker {
	...
	private int displayPageNum = 10; //화면에 보여지는 페이지번호 숫자 갯수(11~20이 보이면, 10)
	...
	//페이지번호 전달을 단순화시킨 클래스 사용을 위한 메소드
...
//	http://localhost:8080/board/read?page=3&perPageNum=20&searchType=title&keyword=홍길동
	public String makeSearch(int page) {
		UriComponents uriComponents = UriComponentsBuilder.newInstance()
				.queryParam("page", page)
				.queryParam("perPageNum", cri.getPerPageNum())
				.queryParam("searchType", ((SerarchCriteria) cri).getSearchType())
		        .queryParam("keyword", ((SerarchCriteria) cri).getKeyword())
	            .build();
